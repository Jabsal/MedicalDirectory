...
export default function SpecialistCard({ specialist, hospitalName, onBookAppointment, onViewDetails }: SpecialistCardProps) {
  const renderStars = (rating: number = 0) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(<Star key={`star-${i}`} className="text-yellow-400 fill-yellow-400 h-4 w-4" />);
    }

    if (hasHalfStar) {
      stars.push(<StarHalf key="half-star" className="text-yellow-400 fill-yellow-400 h-4 w-4" />);
    }
      ...
      <CardContent className="p-6">
        <div className="flex items-start">
          {specialist.imageUrl ? (
            ...
            <motion.div
              ...
              whileHover={{ scale: 1.1 }}
            >
              <DoctorSilhouette className="h-16 w-16" />
            </motion.div>
          )}
          <div className="ml-4">
            <h4 className="text-lg font-bold text-gray-900">{specialist.title || "Dr."} {specialist.firstName} {specialist.lastName}</h4>
            <p className="text-sm text-primary">{specialist.specialty} Specialist</p>
        ...
        <div className="mt-4">
          ...
          <div className="mt-1 flex items-center text-sm text-gray-600">
            ...
            <span>{specialist.address}, {specialist.city}, {specialist.state} {specialist.zipCode}</span>
          </div>
          <div className="mt-1 flex items-center text-sm text-gray-600">
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {specialist.subspecialties?.slice(0, 2).map((subspecialty, index) => (
        ...
        <div className="mt-5 flex flex-col gap-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm text-gray-600">
              {specialist.distance !== null ? formatDistance(specialist.distance) : 'Distance unknown'}
            </span>
          </div>
